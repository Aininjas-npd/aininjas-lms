...
**Big packages.** The zip is streamed: it goes straight to disk as it uploads, then each file inside is piped out of the archive one at a time. Memory stays flat whatever the package weighs — a 1.2 GB course imports with a peak of about 100 MB of RAM, where the old reader needed over 1.5 GB and took the container down. The size limit is therefore about disk, not memory: `MAX_PACKAGE_MB` (default 4096) sets it, and an oversized upload is refused with a sentence that names the limit. Remember the unpacked course needs roughly as much room again, so check **Storage** on the dashboard before importing something large.
...
A learner's progress counts only their own steps, so a course of two student steps plus a lesson plan and an in-class quiz reads 50% after one step, not 25%. Teachers and school admins see the whole path — including the teacher-only steps — without being enrolled, which is how they reach the plan and the projector copy of a lesson. That access is limited to courses offered to **their own school** (a course with no school restriction is offered to everyone); only the AI Ninjas admin can open any course.
