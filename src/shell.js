...
function navFor(user, path = '', pluginNav = []) {
  ...
  if (staff && onesite.quiz.configured) items.push({ href: onesite.quiz.on ? onesite.quiz.prefix + '/admin' : onesite.quiz.public + '/admin', label: 'Assessments', on: on(p => p.startsWith(onesite.quiz.prefix + '/') || p === onesite.quiz.prefix) });
  if (user.role === 'admin') {
...
function shellFor(req) {
  ...
  return {
    ...
    nav: navFor(user, String(req.originalUrl || req.path).split('?')[0], req.app.locals.pluginNavFor ? req.app.locals.pluginNavFor(user) : []),   // originalUrl: inside the proxy mount req.path has lost the prefix
    user: { name: user.name, role_label: ROLE_LABEL[user.role] || 'Learner' },
    profile: '/profile',
    account: user.sso_sub && onesite.accounts.configured ? (onesite.accounts.on ? onesite.accounts.prefix + '/' : onesite.accounts.public + '/') : null,
    logout: '/logout',
  };
}
